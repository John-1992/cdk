...
def operation(op):
	...
	if op == '-' and num1 == '':
		...
		sv.set(op)
		print(All)
	elif len(All) > 1 and op == '-' and All[-1] in ['+','-','×','÷'] and All[-2].isdigit():
		operator = op
		All = All+operator
		sv.set(All)
		print(All)
	elif All != '' and op in ['+','-','×','÷'] and num1 != '' and All[-1].isdigit() and All[-1] not in ['+','-','×','÷']:
		operator = op
		All = All+operator
		sv.set(All)
		print(All)
	if All != '' and op == '=' and num1 != '' and All[-1] not in ['+','-','×','÷'] and All[-1].isdigit():
		print(All)
		operator = op
		All = All.replace('×','*')
		All = All.replace('÷','/')
